...
try{
 ...
 for(const base of ['http://127.0.0.1:4186/demos/computer-assets/','http://127.0.0.1:4187/myportal/demos/computer-assets/']){
  await page.goto(base);await expect(page.locator('.type-card')).toHaveCount(schema.length);await page.evaluate(()=>document.fonts.ready);await page.screenshot({path:'.artifacts/computer-assets-dashboard.png',fullPage:true});
  await page.goto(`${base}inventory.html`);await expect(page.locator('#tableSummary')).toContainText(`${records.length} รายการ`);await expect(page.locator('#output tr')).toHaveCount(10);
  await page.locator('#nextPage').click();await expect(page.locator('#tableSummary')).toContainText(`หน้า 2 / ${Math.ceil(records.length/10)}`);
  ...
  await page.locator('#edit-asset_number').fill('DEMO-NEW');await page.locator('#edit-pc_name').fill('=1+1');await page.locator('#saveAsset').click();await expect(page.locator('#assetModal')).toBeHidden();await expect(page.locator('#tableSummary')).toContainText(`รวมทั้งหมด ${records.length+1}`);
  ...
  await page.goto(`${base}reports.html`);await expect(page.locator('#reportTable tbody tr')).toHaveCount(records.length+1);await page.locator('#replacementOnly').check();await expect(page.locator('#reportTable tbody tr')).toHaveCount(schema.length*2+1);await page.locator('#statusFilter').selectOption('dispose');await expect(page.locator('#reportTable tbody')).toContainText('ไม่พบรายการ');
  await page.goto(`${base}inventory.html?type=office_desk&id=asset-081`);await expect(page.locator('#edit-material')).toHaveValue('ไม้เคลือบเมลามีน');await page.locator('#edit-purchase_price').fill('7654.25');await page.locator('#edit-warranty_end').fill('2029-12-31');await page.locator('#edit-location').fill('ห้องประชุมตัวอย่าง');await page.locator('#saveAsset').click();await expect(page.locator('#assetModal')).toBeHidden();
  await page.reload();await expect(page.locator('#edit-purchase_price')).toHaveValue('7654.25');await expect(page.locator('#edit-warranty_end')).toHaveValue('2029-12-31');await expect(page.locator('#edit-location')).toHaveValue('ห้องประชุมตัวอย่าง');await page.waitForFunction(()=>getComputedStyle(document.querySelector('#assetModal')).opacity==='1');await page.screenshot({path:'.artifacts/office-assets-details.png'});
  await page.goto(`${base}activity.html`);await expect(page.locator('.activity-entry')).toHaveCount(3);
 ...
 await page.goto('http://127.0.0.1:4187/myportal/project.html?id=computer-assets');await expect(page.locator('#project-title')).toHaveText('ระบบทรัพย์สินทร์');await expect(page.getByRole('heading',{name:'ผลลัพธ์และสิ่งที่ได้เรียนรู้',exact:true})).toHaveCount(0);
 assert.deepEqual(errors,[]);assert.deepEqual(unsafe,[]);
 console.log('IT and office assets: all 13 field sets, filters/pagination, details, decimal prices/warranty dates, edits, duplicate prevention, CSV escaping/filtering, reports, history, reset, mobile/tablet, CSP and GitHub subpath passed.');
